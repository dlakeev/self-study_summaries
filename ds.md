...
12. [Нейронные сети](#12)
    1. [Искусственный нейрон](#12.1)
    2. [Функции активации](#12.2)
    3. [Построение сети на keras](#12.10)
...
Шаг градиентного спуска осуществляется для _всех_ параметров модели _одновременно_.
...
- Если $||w_{k} - w_{k-1}|| < \epsilon$, то завершить - можно выйти из цикла обучения, если коэффициенты изменяются не сильно (на маленькое значение эпсилон).
...
Регуляризация - накладывание _штрафа_ на модель за излишнюю сложность. Она признана бороться с _большими весами_ модели.

### Ridge

$$Q(w, X) + \lambda ||w||^2 \rightarrow min_w$$

где $||w||^{2} = \sqrt{a_{1}^{2} + a_{2}^{2}+ \ldots + a_{n}^{2}}^{2} = a_{1}^{2} + a_{2}^{2}+ \ldots + a_{n}^{2}$,

а $Q(w, X) = ||y-Xw||^{2}$ $(MSE)$.

### Lasso

Зануляет _бесполезные_ характеристики в данных.

$$Q(w, X) + \lambda ||w|| \rightarrow min_w$$

где $||w|| = |w_{1}| + |w_{2}| + \ldots + |w_{n}|$,
...
$$Q(w, X) + \lambda_{1} ||w||_{1} +  \lambda_{2} ||w||_{2}^{2} \rightarrow min_w$$
...
___
# 11. Рекомендательные системы <a id="11.1"></a>
## 11.1 Косинусная похожесть <a id="11.1"></a>

Пользователю можно рекомендовать товар или услугу, исходя из его _похожести_ на других пользователей.

<p align="center">
  <img src="img/Косинусная похожесть.png">
</p>

Похожесть можно сравнивать:
* Content based (описание просмотренных фильмов);
* Collaborative Filtering (купленные товары).

___
# 12. Нейронные сети <a id="12"></a>
## 12.1 Искусственный нейрон <a id="12.1"></a>
...
Нейрон представляет из себя взвешенную сумму и является _линейной_ моделью:

$$y = \sum{XW} + bias$$

, где $y$ может изменяться от $-∞$ до $+∞$. И значит _всегда_ этот нейрон будет пропускать через себя сигнал, _не имея границ активации_. **_Функция активация_** нужна для того, чтобы пропустить сигнал нейрона _дальше_ или проигнорировать его.
___
## 12.2 Функции активации <a id="12.2"></a>
### Линейная функция (linear)
$$A(x)=x$$ 


<p align="center">
  <img src="img/linear_activation.png">
</p>

Преимущество:
* Меньше времени на вычисления

Проблемы:
* Для этой функции производная всегда одинакова в любой точке, а это значит, что градиент никак не связан с входом $x$. Градиент - постоянный вектор, а спуск идет по постоянному градиенту. Если есть ошибочное предсказание, то изменения, сделанные обратным распространением ошибки, тоже постоянны вне зависимости от входа.
* Нет смысла ставить подряд слои с линейной активацией.
...
<p align="center">
  <img src="img/sigmoid_activation.png">
</p>

Преимущества:
* Сигмоида — _нелинейная функция_, а комбинация таких функций производит тоже нелинейную функцию. А значит проблема линейной функция здесь неактуальна, мы можем обучать подряд _много слоев_ и это будет иметь смысл.
* В диапазоне значений сигнала от -2.5 до 2.5 значения активации меняются очень быстро. А значит, что любое изменение значения сигнала в этой области повлечет существенное изменение значения активации. 
* Сигмоида идеально подходит для задач _бинарной классификации_. Она стремится привести значения к одной из сторон кривой, по итогу получаем четкие границы предсказания.
* У сигмоиды фиксированный диапазон значений функции — [0,1], а линейная функция обладает диапазоном $(-∞, +∞)$. А значит в случае с сигмоидой _не будет ошибок_ с большими значениями активаций.

Проблема:
* При приближении к концам сигмоиды значения активации очень слабо изменяются. Это означает, что градиент в таких областях имеет очень мальнькие значения. А из-за этого рождается проблема **_затухающих градиентов_** - градиент получается настолько маленьким или же вообще нулевым, что обучение замораживается или же идет очень медленно.
...
<p align="center">
  <img src="img/tanh_activation.png">
</p>

Гиперболический тангенс очень похож на _сигмоиду_, только вот производная у него _чуть больше_, но все остальные плюсы и минусы остаются.
...
<p align="center">
  <img src="img/relu_activation.png">
</p>

Преимущество или проблема:
* Если имеется большую нейронную сеть с огромным количеством нейронов, то использование сигмоиды или гиперболического тангенса повлекут за собой активацию _буквально всех нейронов_. А в идеале хочется, чтобы некоторые нейроны не были активированы, чтобы сеть была легче. **_ReLu_** может это сделать. К примеру, если есть нейросеть со случайно инициализированными весами, то 50% активаций равны 0 по ReLU, однако градиент на этой части равен 0. А значит, веса сети _не будут изменяться_ во время обучения.
...
<p align="center">
  <img src="img/leaky_relu_activation.png">
</p>

Имеет смысл для для решения проблемы нулевого градиента (ReLU).

### ELU

<p align="left">
  <img src="img/elu_formula.png">
</p>

<p align="center">
  <img src="img/elu_activation.png">
</p>

Так же имеет смысл для для решения проблемы нулевого градиента (ReLU).

### Как выбрать функцию активации

В промежуточных слоях нет очевидного ответа на этот вопрос. Нужно опытным путем выбрать ту функцию, которая ведет к более быстрому обучению.

Есть только однозначный ответ касаемо функции активации на последнем слое:
* Если задача классификации, то самый лучший кандидат - sigmoid, т.к. выдает по сути вероятность быть одним классом или другим.
* Если задача регрессии, то ReLU - если не может быть отрицательного целевого значения или же линейная активация.

Кроме того, можно использоваться кастомные функции активации.

| **Функция активации** | **Плюсы**                                                                                                                                                         | **Минусы**                                                                                                     |
|-----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------|
| linear                | 1. Меньше времени на вычисления                                                                                                                                   | 1. Линейная функция 2. Нефиксированный диапазон значений функции — [-∞,+∞]                                     |
...
| ELU                   | 1. Нет проблемы нулевых градиентов 2. Имеются негативные выходы, что помогает нейронке направлять веса в нужное напрвление                                        | 1. Больше времени на вычисления 2. Нефиксированный диапазон значений функции 3. Нужно подбирать значение alpha |

___
## 12.10 Построение сети на keras <a id="12.10"></a>
### Алгоритм построения
...
* Прямой проход в сети - от input к output для получения предсказания
* Обратный проход (**_Backpropagation_**) - высчитываются градиенты, чтобы в каждом слое изменить веса для уменьшения ошибки.

Для этого используется **_chain rule_**.
